...
import { addToCart, deleteOnePizza, removePizzas, clearCart } from '../redux/actions/cart'
function Cart() {
    const dispatch = useDispatch()
    const { items, totalPrice, totalCount } = useSelector(({ cart }) => cart)

    const newItems = Object.keys(items).map((item) => items[item].items[0])

    const addOnePizza = (obj) => {
                ...
                <div className="content__items">
                    {
                        newItems.map((item) =>
                            <CartItem
                                ...
                                removePizzasList={removePizzasList}
                                name={item.name}
                                price={item.price}
